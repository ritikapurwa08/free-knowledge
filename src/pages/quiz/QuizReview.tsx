  ...
  if (!state) {
    return (
        <div className="flex items-center justify-center min-h-screen">
            ...
            <button onClick={() => navigate('/quiz')} className="text-primary ml-2">Go back</button>
        </div>
    );
  }
    ...
    <div className="min-h-screen bg-background-light dark:bg-background-dark pb-24">
      ...
      <section className="p-4">
        <div className="grid grid-cols-3 gap-3">
          <div className="flex flex-col items-center justify-center rounded-xl bg-white dark:bg-[#1a2230] p-4 shadow-sm border border-gray-100 dark:border-gray-800">
            ...
            <p className="text-xl font-bold text-[#111318] dark:text-white">{score}/{totalQuestions}</p>
          </div>
          <div className="flex flex-col items-center justify-center rounded-xl bg-white dark:bg-[#1a2230] p-4 shadow-sm border border-gray-100 dark:border-gray-800">
            ...
            <p className="text-xl font-bold text-[#111318] dark:text-white">{Math.floor(quizData.timeLimit / 60)}m</p>
          </div>
          <div className="flex flex-col items-center justify-center rounded-xl bg-white dark:bg-[#1a2230] p-4 shadow-sm border border-gray-100 dark:border-gray-800">
            ...
            <p className="text-xl font-bold text-[#111318] dark:text-white">{percentage}%</p>
          </div>
        </div>
        <div className="mt-4 flex items-center justify-center gap-2 rounded-lg bg-primary/10 p-3 text-center border border-primary/20">
          ...
          <p className="text-sm font-medium text-primary">
            {percentage >= 80 ? "Great job! You're mastering this topic." :
      ...
      <div className="flex flex-col gap-6 px-4 pb-6">
        ...
        {quizData.questions.map((q, index) => {
            ...
            return (
                <div key={q.id || index} className="rounded-xl bg-white dark:bg-[#1a2230] shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden">
                  <div className="p-5 border-b border-gray-100 dark:border-gray-800">
                    <div className="flex items-center justify-between mb-3">
                      <span className="bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 text-xs font-bold px-2 py-1 rounded">Question {index + 1}</span>
                      {isSkipped ? (
                         <span className="flex items-center gap-1 bg-gray-400 text-white text-xs font-bold px-2 py-1 rounded shadow-sm">
                            <span className="material-symbols-outlined text-[14px]">remove</span> Skipped
                        </span>
                      ) : isCorrect ? (
                  ...
                  <div className="p-4 flex flex-col gap-3">
                    ...
                    {!isSkipped && (
                        <div className={cn(
                            ...
                            <div className="flex flex-col">
                                <p className="text-sm font-bold text-gray-900 dark:text-white">{q.options[userAnswer]}</p>
                                <p className={cn("text-xs font-medium mt-0.5", isCorrect ? "text-green-700 dark:text-green-400" : "text-red-600 dark:text-red-400")}>Your Answer</p>
                            </div>
                        </div>
                    )}
                    ...
                    {(!isCorrect || isSkipped) && (
                        <div className="flex items-start gap-3 rounded-lg border-2 border-green-500 bg-green-50 dark:bg-green-900/20 p-3">
                            ...
                            <div className="flex flex-col">
                                <p className="text-sm font-bold text-gray-900 dark:text-white">{q.options[q.correctAnswer]}</p>
      ...
      <div className="fixed bottom-0 left-0 w-full bg-white dark:bg-[#1a2230] p-4 border-t border-gray-100 dark:border-gray-800 shadow-[0_-4px_12px_rgba(0,0,0,0.05)] z-40 flex gap-4">
        ...
        {nextQuiz && (
            <button
            onClick={() => navigate('/quiz/attempt', { state: { quizData: nextQuiz } })}
            ...
            >
            Next Set <span className="material-symbols-outlined text-[18px]">arrow_forward</span>
            </button>
        )}
      </div>
    </div>
